...
$(document).ready(function(){
   ...
   $("#add_sub_condition").click(function() {// on click button add sub condition row
      ...
      if (i == 1) {// check first row
         ...
         html += '<td><input type="hidden" name="condition_type_row[]" class="condition_type_row" value="main_row_sub_con"><button data_row_id ="row' + i + '"  type="button" name="remove" class="btn btn-danger btn-sm remove">X</button>     <button data_row_id ="row' + i + '" row_id = "' + i + '"  type="button" name="add_sub_con" style="background-color:#e0abff;border:1px solid #e0abff;" class="btn btn-primary btn-sm add_sub_con">+</button></td>';
        ...
        function(fileData) {
            ...
            .done(function(data) {
               if(!data.error){
                  ...
                  let cell_data = webdatarocks.getSelectedCell();
                  if(cell_data == null){
                     Swal.fire({
                        title: 'กรุณาเลือกเซลล์ที่นับจำนวนรายการ',
                        icon: 'warning'
                     })
                  }
                  else{
                     if(isNumeric(cell_data.label)){
                        $("#alert_input_count_record").val(cell_data.label);
                        webdatarocks.removeSelection();
                        if($("#alert_input_count_record").val().length == 0){
                           Swal.fire({
                              title: 'กรุณาเลือกเซลล์ที่นับจำนวนรายการ',
                              icon: 'warning'
                           })
...
function popolated_condition_builder_form(HTTP_post_form){
   ...
   for(row_con_type in HTTP_obj.condition_type_row){
      if(HTTP_obj.condition_type_row[row_con_type] == "main_con"){
         ...
         if (i == 1) {// check first row
            html += '<tr name="row' + i + '" id="row' + i + '" sub_con_row="' + i + '">';
            html += '<td><input type="hidden" name="condition_type_row[]" class="condition_type_row" value="main_row_sub_con"><button data_row_id ="row' + i + '"  type="button" name="remove" class="btn btn-danger btn-sm remove">X</button>     <button data_row_id ="row' + i + '" row_id = "' + i + '" style="background-color:#e0abff;border:1px solid #e0abff;" type="button" name="add_sub_con" class="btn btn-primary btn-sm add_sub_con">+</button></td>';
            html += '<td colspan="1"></td>';
            html += '<td colspan="1"><label>เลือกตัวเชื่อมเงื่อนไข</label></td>';
            html += '<td colspan="1"> <input class="form-control form-control-sm sub_con_optlist" type="text" name="sub_con_optlist[]" readonly value=""></input></td>';
            html += '<td colspan="1"></td>';
...
function generate_table_WBS_task(data) {
   ...
   let html = '';
   // table thead
   html += '<div style="overflow: auto;width:100%;height:70vh;"><table class="table table-sm table-bordered tb-result"><thead class="text-center text-light tb_head"><tr><th width="5%">#</th><th width="10%">#</th><th width="13%">ลำดับที่</th><th width="45%">รายการ</th><th width="27%">WBS(จำนวนรายการ)</th></tr></thead>';
   ...
   Object.keys(data).forEach(function(k) {
      if (k == "") { //<input class="check_wbs_row" type="checkbox" name="check_wbs_row[]" value="' + k + '">
         html += '<tr style="background-color:#d2b0ff;" class="header" data_wbs="null_value"><td><input class="WBS_tr" type="checkbox" value="null_wbs"></td><td><span class="btn btn-primary btn-sm normal_btn">-</span></td><td></td><td></td><td>' + k + '('+ data[k].length+')</td></tr>';
         let i = 1;
         ...
         html += '<tr style="background-color:#d2b0ff;" class="header" data_wbs="' + k + '"><td><input class="WBS_tr" type="checkbox" value="' + k + '"></td><td><span class="btn btn-primary btn-sm normal_btn">-</span></td><td></td><td></td><td>' + k + '('+ data[k].length+')</td></tr>';
         let i = 1;
         Object.keys(data[k]).forEach(function(sub_loop) {
            html += '<tr style="background-color:#e9d9ff;">';
...
function generate_table_result(data) {// generate table result
   ...
   let html = '';
   // table thead
   html += '<div style="overflow: auto;width:100%;height:70vh;"><table class="table table-sm table-bordered table-striped tb-result"><thead class="text-center text-light tb_head"><tr><th><input type="checkbox" id="other_task_check_all"></th><th>#</th>'; 
