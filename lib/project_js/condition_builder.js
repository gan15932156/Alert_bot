   ...
   $("#send_to_webdatarocks").click(function(){
      ...
      if($("#task_id").val() == "null" && $("#token_line_id").val() == null){
         ...
         .done(function(data) { // response
            ...
            //console.log(data)
            getJSONData(data.WBS_select_sql, data.json_count);
...
function generate_table_WBS_task(data) {
   ...
   let html = '';
   // table thead
   html += '<div style="overflow: auto;width:100%;height:70vh;"><table class="table table-sm table-bordered table_wbs"><thead class="text-center bg-primary"><tr><th width="5%">#</th><th width="10%">#</th><th width="13%">ลำดับที่</th><th width="45%">รายการ</th><th width="27%">WBS(จำนวนรายการ)</th></tr></thead>';
   ...
   Object.keys(data).forEach(function(k) {
      if (k == "") { //<input class="check_wbs_row" type="checkbox" name="check_wbs_row[]" value="' + k + '">
         html += '<tr style="background-color:lightblue;" class="header" data_wbs="null_value"><td></td><td><span class="btn btn-primary btn-sm">-</span></td><td></td><td></td><td>' + k + '('+ data[k].length+')</td></tr>';
         let i = 1;
         Object.keys(data[k]).forEach(function(sub_loop) {
            html += '<tr>';
            html += '<td><input class="row_id" type="checkbox" name="row_id[]" value="' + data[k][sub_loop]["primary_key"]+ '"></td>';
            html += '<td><b>' + i + '</b></td>';
         ...
         html += '<tr style="background-color:lightblue;" class="header" data_wbs="' + k + '"><td></td><td><span class="btn btn-primary btn-sm">-</span></td><td></td><td></td><td>' + k + '('+ data[k].length+')</td></tr>';
         let i = 1;
         Object.keys(data[k]).forEach(function(sub_loop) {
            html += '<tr>';
            html += '<td><input class="row_id" type="checkbox" name="row_id[]" value="' + data[k][sub_loop]["primary_key"] + '"></td>';
...
function getJSONData(sql, fields) { // เรียกข้อมูลจากฐานข้อมูล
   ...
   if(typeof sql == "object"){
      $.ajax({
         url: "Http_request/get_populate_data_webdatarocks.php",
         method: "POST",
         async: false,
         dataType: "JSON",
         data: {
